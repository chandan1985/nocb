...
function bc_menus() {
	add_management_page('Batch Categories', 'Batch Categories', 'administrator', 'batch_categories', 'bc_admin_screen');
}
add_action('admin_menu', 'bc_menus');

function bc_admin_head() {
	if ( isset($_GET['page']) && $_GET['page']  == 'batch_categories' ) {
		echo '
...
function bc_admin_screen() {
	...
	$perpage = ( in_array($_GET['perpage'], $per_page) ) ? $_GET['perpage'] : 100;
	...
	';
	foreach ( $per_page as $dispcount => $value ) {
		$selected = ( $perpage == $value ) ? ' selected="selected"' : '';
	...
	if ( !empty($_GET['cat']) || !empty($_GET['s']) || !empty($_GET['t']) ) {
		...
		$q = "paged=$paged&posts_per_page=$perpage&orderby=$orderby&order=$order&post_type=post";
		if ( !empty($_GET['cat']) ) {
			$cat = get_cat_name(intval($_GET['cat']));
			//$q .= "&category_name=$cat";
			$q .= "&cat=" . intval($_GET['cat']);
		}
