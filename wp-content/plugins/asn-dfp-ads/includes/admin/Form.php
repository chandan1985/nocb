...
Abstract Class Form {
  ...
  public function yes_no_dropdown( $args ) {
		// Why is it so nested?
		$args = $args[0];
		// Field values
		$id    = $this->options_str . '[' . $args['id'] . ']';
		...
		<div>
			<select id="<?php _e( $id, 'dfp-ads' ); ?>" name="<?php _e( $id, 'dfp-ads' ); ?>">
				<option value="1" <?php if($value==1){?> selected="selected" <?php } ?>>Yes</option>
        <option value="0" <?php if($value==0){?> selected="selected" <?php } ?>>No</option>
			</select>
			<?php if ( isset( $args['description'] ) ) { ?>
				<p><em><?php _e( $args['description'], 'dfp-ads' ); ?></em></p>
			<?php } ?>
