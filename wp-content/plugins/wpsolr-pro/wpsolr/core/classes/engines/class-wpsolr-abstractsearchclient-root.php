...
TAG;
	...
	public function get_suggestions_keywords_data( WPSOLR_Query $wpsolr_query, $suggestion, $query ) {
		...
		if ( $this->is_galaxy_slave ) {
			// Filter results by the slave context
			$contexts['context_blog_name'] = [ $this->galaxy_slave_filter_value ];
		}
	...
	public function set_query_select( WPSOLR_Query $wpsolr_query ) {
		...
		// Set the event tracking
		$this->_set_event_tracking( $wpsolr_query );
	...
	public function get_results_data( WPSOLR_Query $wpsolr_query, $widget_options = [], $is_use_cache = true ) {
		...
		if ( WPSOLR_Service_Container::getOption()->get_search_is_event_tracking() ) {
			$data['event_tracking'] = [ 'query_id' => $results->get_event_tracking_query_id(), ];
		}
	...
	function add_filter_query_fields(
		...
		if ( $this->is_galaxy_slave ) {
			...
			$filter_query_fields[ WpSolrSchema::_FIELD_NAME_BLOG_NAME_STR ] = [ $this->galaxy_slave_filter_value ];
		}
