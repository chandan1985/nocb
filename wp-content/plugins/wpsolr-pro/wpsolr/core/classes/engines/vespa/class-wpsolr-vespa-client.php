...
trait WPSOLR_Vespa_Client {
	...
	function get_index_settings() {
		...
		switch ( $module ) {
			...
			case WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_OPENAI:
				// https://vespa.io/developers/vespa/current/retriever-vectorizer-modules/text2vec-openai.html#example-class-configuration
				$class_schema['moduleConfig'] = [
					WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_OPENAI => [
						'model'              => $this->config['extra_parameters']['index_vespa_openai_config_model'],
						'modelVersion'       => $this->config['extra_parameters']['index_vespa_openai_config_model_version'],
						'type'               => $this->config['extra_parameters']['index_vespa_openai_config_type'],
						'vectorizeClassName' => false,
					],
				];
				break;

			case WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE:
				// https://vespa.io/developers/vespa/current/retriever-vectorizer-modules/text2vec-huggingface.html#how-to-configure
				$config = [
					'options'            => [
						'waitForModel' => true,
						'useGPU'       => true,
				...
				$model = $this->config['extra_parameters']['index_vespa_huggingface_config_model'];
				if ( ! empty( $this->config['extra_parameters']['index_vespa_huggingface_config_model_query'] ) ) {

					$config['passageModel'] = $model;
					$config['queryModel']   = $this->config['extra_parameters']['index_vespa_huggingface_config_model_query'];
				...
				$class_schema['moduleConfig'] = [
					WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE => $config,
				];

				break;

			case WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_COHERE:
				// https://vespa.io/developers/vespa/current/retriever-vectorizer-modules/text2vec-cohere.html
				$class_schema['moduleConfig'] = [
					WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_COHERE => [
						'model'              => $this->config['extra_parameters']['index_vespa_cohere_config_model'],
						'vectorizeClassName' => false,
					],
				];
		...
		if ( ! empty( $this->config['extra_parameters']['index_vespa_openai_config_type_qna'] ) &&
		     ! empty( $this->config['extra_parameters']['index_vespa_openai_config_model_qna'] ) &&
		     ! empty( $this->config['extra_parameters']['index_vespa_openai_config_model_version_qna'] )
		) {
			$class_schema['moduleConfig'][ WPSOLR_Vespa_Constants::MODULE_QNA_OPENAI ] = [
				// https://vespa.io/developers/vespa/modules/reader-generator-modules/qna-openai
				'model'            => sprintf( "%s-%s-%s",
					$this->config['extra_parameters']['index_vespa_openai_config_type_qna'],
					$this->config['extra_parameters']['index_vespa_openai_config_model_qna'],
					$this->config['extra_parameters']['index_vespa_openai_config_model_version_qna']
				),
				'maxTokens'        => 16,
				'temperature'      => 0.0,
				'topP'             => 1,
	...
	static public function get_analysers() {
		...
		return [
			...
			WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_OPENAI        => [ 'label' => 'OpenAI Embeddings API (text2vec-openai)', ],
			WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE   => [ 'label' => 'Hugging Face Embeddings API (text2vec-huggingface)', ],
			WPSOLR_Vespa_Constants::MODULE_TEXT_2_VEC_COHERE        => [ 'label' => 'Cohere Embeddings API (text2vec-cohere)', ],
