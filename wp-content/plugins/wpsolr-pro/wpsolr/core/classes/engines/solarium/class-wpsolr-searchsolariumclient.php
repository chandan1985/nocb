...
class WPSOLR_SearchSolariumClient extends WPSOLR_AbstractSearchClient {
	...
	protected function _log_query_as_string() {
		...
		$edismax = [];
		foreach ( $this->query_select->getEDisMax()->getBoostQueries() ?? [] as $boost_query ) {
			$edismax['boost_query'][ $boost_query->getKey() ] = $boost_query->getQuery();
		}
		if ( ! empty( $this->query_select->getEDisMax()->getQueryFields() ) ) {
			$edismax['boost_query_fields'] = $this->query_select->getEDisMax()->getQueryFields();
		}
