...
trait WPSOLR_Weaviate_Client {
	...
	function get_index_settings() {
		...
		switch ( $module ) {
			...
			case WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_OPENAI:
				// https://weaviate.io/developers/weaviate/current/retriever-vectorizer-modules/text2vec-openai.html#example-class-configuration
				$class_schema['moduleConfig'] = [
					WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_OPENAI => [
						'model'              => $this->config['extra_parameters']['index_weaviate_openai_config_model'],
						'modelVersion'       => $this->config['extra_parameters']['index_weaviate_openai_config_model_version'],
						'type'               => $this->config['extra_parameters']['index_weaviate_openai_config_type'],
						'vectorizeClassName' => false,
					],
				];
				break;
			...
			case WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE:
				// https://weaviate.io/developers/weaviate/current/retriever-vectorizer-modules/text2vec-huggingface.html#how-to-configure
				$config = [
					'options'            => [
						'waitForModel' => true,
						'useGPU'       => true,
				...
				$model = $this->config['extra_parameters']['index_weaviate_huggingface_config_model'];
				if ( ! empty( $this->config['extra_parameters']['index_weaviate_huggingface_config_model_query'] ) ) {

					$config['passageModel'] = $model;
					$config['queryModel']   = $this->config['extra_parameters']['index_weaviate_huggingface_config_model_query'];

				} else {
					$config[ ( false !== strpos( $model, 'http', ) ) ? 'endpointURL' : 'model' ] = $model;
				}

				$class_schema['moduleConfig'] = [
					WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE => $config,
				];
			...
			case WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_COHERE:
				...
				$class_schema['moduleConfig'] = [
					WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_COHERE => [
						'model'              => $this->config['extra_parameters']['index_weaviate_cohere_config_model'],
						'vectorizeClassName' => false,
					],
				];

				$class_schema['vectorIndexConfig'] = [
					'distance' => 'dot'
				];
			...
			case WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_PALM:
				// https://weaviate.io/developers/weaviate/modules/retriever-vectorizer-modules/text2vec-palm
				$class_schema['moduleConfig'] = [
					WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_PALM => [
						'modelId'            => $this->config['extra_parameters']['index_weaviate_palm_config_model'],
						'apiEndpoint'        => 'us-central1-aiplatform.googleapis.com',
						// https//us-central1-aiplatform.googleapis.com/v1/projects/weaviate-374814/locations/us-central1/endpoints/textembedding-gecko-001:predict
						'projectId'          => json_decode( $this->config['extra_parameters'][ WPSOLR_Hosting_Api_Abstract::FIELD_NAME_FIELDS_INDEX_KEY_JSON ], true )['project_id'],
						'vectorizeClassName' => false,
					],
				];
				break;
		...
		if ( ! empty( $this->config['extra_parameters']['index_weaviate_openai_config_type_qna'] ) &&
		     ! empty( $this->config['extra_parameters']['index_weaviate_openai_config_model_qna'] ) &&
		     ! empty( $this->config['extra_parameters']['index_weaviate_openai_config_model_version_qna'] )
		) {
			$class_schema['moduleConfig'][ WPSOLR_Weaviate_Constants::MODULE_QNA_OPENAI ] = [
				// https://weaviate.io/developers/weaviate/modules/reader-generator-modules/qna-openai
				'model'            => sprintf( "%s-%s-%s",
					$this->config['extra_parameters']['index_weaviate_openai_config_type_qna'],
					$this->config['extra_parameters']['index_weaviate_openai_config_model_qna'],
					$this->config['extra_parameters']['index_weaviate_openai_config_model_version_qna']
				),
				'maxTokens'        => 16,
				'temperature'      => 0.0,
				'topP'             => 1,
	...
	static public function get_analysers() {
		...
		return [
			...
			WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_OPENAI        => [ 'label' => 'OpenAI Embeddings API (text2vec-openai)', ],
			WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE   => [ 'label' => 'Hugging Face Embeddings API (text2vec-huggingface)', ],
			WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_COHERE        => [ 'label' => 'Cohere Embeddings API (text2vec-cohere)', ],
			WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_PALM          => [ 'label' => 'Google PaLM Embeddings API (text2vec-palm)', ],
