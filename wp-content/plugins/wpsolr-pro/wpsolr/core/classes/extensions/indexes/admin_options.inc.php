    ...
    jQuery(document).ready(function ($) {
        ...
        function show_hide_fields(engine, hosting_api_id, current_analyser_id, is_form_validation) {
            ...
            jQuery.each(g_ui_api_fields[g_field_name_hosting_apis][hosting_api_id][g_field_name_fields], function (field_name, field_properties) {
                ...
                toggle_openai_fields(engine, current_analyser_id);
        ...
        function toggle_openai_fields(current_engine, current_analyser_id) {

            if ('<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_AbstractEngineClient::ENGINE_WEAVIATE ); ?>' === current_engine) {
                openai_fields_el.hide();
                huggingface_fields_el.hide();
                cohere_fields_el.hide();
                google_palm_fields_el.hide();
                if ('<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_OPENAI ); ?>' === current_analyser_id) {
                    openai_fields_el.show();
                }

                if ('<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_HUGGINGFACE ); ?>' === current_analyser_id) {
                    huggingface_fields_el.show();
                }

                if ('<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_COHERE ); ?>' === current_analyser_id) {
                    cohere_fields_el.show();
                }

                if ('<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_Weaviate_Constants::MODULE_TEXT_2_VEC_PALM ); ?>' === current_analyser_id) {
                    google_palm_fields_el.show();
                }
            }
...
<div id="solr-hosting-tab">
    ...
    <div id="solr-results-options" class="wdm-vertical-tabs-content">
		...
		<?php if ( $is_new_index && ! $option_object->has_index_type_temporary() ) {
			...
			$btn_title = "Connect to your search server or search service";
			?>
            ...
            <form action="options.php" method="POST" id='settings_conf_form'>
				...
				foreach ( ( isset( $option_data[ WPSOLR_Option::OPTION_INDEXES_INDEXES ] ) ? $option_data[ WPSOLR_Option::OPTION_INDEXES_INDEXES ] : [] ) as $index_indice => $index ) {
                    ...
                    <div
                        ...
                        <?php WPSOLR_Escape::echo_esc_attr( WPSOLR_AbstractEngineClient::ENGINE_GOOGLE_RETAIL ); ?>
						...
						<?php include_edit_field( 'textedit', WPSOLR_Hosting_Api_Abstract::FIELD_NAME_FIELDS_INDEX_WEAVIATE_GOOGLE_PALM_CONFIG_MODEL, $is_index_readonly, $is_new_index, $option_name, $option_data, $index_indice, $subtab, false, false ); ?>
