	...
	if ( isset( $_POST['security'] ) && wp_verify_nonce( $_POST['security'], WPSOLR_NONCE_FOR_DASHBOARD ) ) {
		...
		try {
			...
			if ( ! empty( $endpoint ) ) {
				...
				if ( WPSOLR_Hosting_Api_Solr_Solrfarm::HOSTING_API_ID === $index_hosting_api_id ) {
					$path = $url['path'];
				}

				if ( ( WPSOLR_Hosting_Api_Solrcloud_Searchstax::HOSTING_API_ID === $index_hosting_api_id ) ||
				     ( WPSOLR_Hosting_Api_Solr_Solrfarm::HOSTING_API_ID === $index_hosting_api_id ) ) {
					$output_data['index_path'] = $path;
				}
			}
			...
			switch ( $index_hosting_api_id ) {
				case WPSOLR_Hosting_Api_Elasticsearch_Amazon::HOSTING_API_ID:
				case WPSOLR_Hosting_Api_OpenSearch_Amazon::HOSTING_API_ID:
				case WPSOLR_Hosting_Api_Amazon_Personalize::HOSTING_API_ID:
					$is_hosting_aws = true;
					break;
