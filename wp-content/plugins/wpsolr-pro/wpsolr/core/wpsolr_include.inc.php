...
function wpsolr_enqueue_search() {
	...
	foreach ( $views as $view_uuid => $view ) {
		...
		$is_autocomplete = ! WPSOLR_Service_Container::getOption()->get_cross_domain_is_galaxy_slave();
		if ( $is_autocomplete ) {
		...
		$container_overlay         = WPSOLR_Service_Container::getOption()->get_option_theme_ajax_page_overlay_jquery_selectors( true );
		$search_is_event_tracking  = false;
		$search_is_event_tracking = WPSOLR_Service_Container::getOption()->get_search_is_event_tracking();
		$event_tracking_results  = $search_is_event_tracking ? WPSOLR_Service_Container::getOption()->get_search_event_tracking_results_jquery_selector() : '';
		$event_tracking_query_id = ( $search_is_event_tracking && ( $wp_query instanceof WPSOLR_Query ) && $wp_query->wpsolr_get_results() ) ? $wp_query->wpsolr_get_results()->get_event_tracking_query_id() : '';
		$ajax_delay_ms           = WPSOLR_Service_Container::getOption()->get_option_theme_ajax_delay_ms();
		$event_tracking          = [
			'WPSOLR_AJAX_EVENT_TRACKING_ACTION'            => WPSOLR_AJAX_EVENT_TRACKING_ACTION,
			'SEARCH_PARAMETER_RESULTS_QUERY_ID'            => WPSOLR_Query_Parameters::SEARCH_PARAMETER_RESULTS_QUERY_ID,
			'SEARCH_PARAMETER_RESULTS_POSITION'            => WPSOLR_Query_Parameters::SEARCH_PARAMETER_RESULTS_POSITION,
			'SEARCH_PARAMETER_EVENT_TRACKING_NAME'         => WPSOLR_Query_Parameters::SEARCH_PARAMETER_EVENT_TRACKING_NAME,
			'SEARCH_PARAMETER_EVENT_TRACKING_CLICK_RESULT' => WPSOLR_Query_Parameters::SEARCH_PARAMETER_EVENT_TRACKING_NAME_CLICK_RESULT,
			'css_event_tracking_results'                   => $event_tracking_results,
			'event_tracking_query_id'                      => $event_tracking_query_id,
		];
