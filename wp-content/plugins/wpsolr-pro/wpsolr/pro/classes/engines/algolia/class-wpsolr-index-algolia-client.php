...
/**
 * Class WPSOLR_IndexAlgoliaClient
 *
 * @property SearchClient $search_engine_client
 */
class WPSOLR_Index_Algolia_Client extends WPSOLR_AbstractIndexClient {
...
TAG;
	...
	public function send_posts_or_attachments_to_solr_index( $documents, $is_error = false ) {
		...
		$formatted_docs = $this->search_engine_client_prepare_documents_for_update( $index->getIndexName(), $documents );
		...
		try {
			$response = $this->search_engine_client->multipleBatch( $formatted_docs );

			/**
			 * Send events at most once to each index
			 *
			 * $index_name       = $index->getIndexName();
			 * $formatted_events = $this->search_engine_client_prepare_events_for_update( $index->getIndexName(), $documents );
			 * if ( ! empty( $formatted_events ) ) {
			 * $this->event_engine_client = InsightsClient::create(
			 * $this->config['username'],
			 * $this->config['password']
			 * );
			 *
			 * $this->event_engine_client->sendEvents( $formatted_events );
			 *
			 * foreach ( $documents as $document ) {
			 * if ( 'post_type' === $document['meta_type_s'] ) {
			 * switch ( $document['type'] ) {
			 ...
			 * $event_indices = get_post_meta( $document['id'], static::CUSTOM_FIELD_NAME_EVENT_INDICES, true ) ?? [];
			 * if ( empty( $event_indices ) || ( ! in_array( $index_name, $event_indices ) ) ) {
			 * $event_indices[] = $index_name;
			 * update_post_meta( $document['id'], static::CUSTOM_FIELD_NAME_EVENT_INDICES, $event_indices );
			 * }
			 * }
			 * }
			 * }
			...
			throw new \Exception( sprintf( "(Algolia) \"%s\"\n", $e->getMessage() ) );
		}
	...
	protected function search_engine_client_delete_all_documents( $post_types = null, $site_id = '' ) {
		...
		if ( ( is_null( $post_types ) || empty( $post_types ) ) && ( empty( $site_id ) ) ) {
			...
			/**
			 * https://www.algolia.com/doc/api-reference/api-methods/delete-by/#examples
	...
	protected function search_engine_client_get_count_document( $site_id = '' ) {
		...
		$results = $this->get_search_index()->search( '', $params );
	...
	/**
	 * https://www.algolia.com/doc/guides/managing-results/refine-results/geolocation/how-to/filter-results-around-a-location/#dataset
	 *
	 * @inheritDoc
	 */
	public function get_geolocation_field_value( $field_name, $lat, $long ) {
		...
		return [
			'field_name'  => static::FIELD_NAME_GEOLOC, // Algolia uses a default field geolocation
	...
	public function set_field_is_exist( array &$document_for_update, string $field_name, string $is_exists ) {
		// Manage the 'not exists': https://www.algolia.com/doc/guides/managing-results/refine-results/filtering/how-to/filter-by-null-or-missing-attributes/
	...
	protected function _transform_event_tracking( \WP_Post $post, array $event, string $event_label ) {
		/**
		 * Transform: https://www.algolia.com/doc/api-reference/api-methods/send-events/
		 */
	...
	protected function _transform_event_tracking_click_search_result( \WP_Post $post, array $event, array $transformed_event ) {
		...
		$position                       = 1 + intval( $event[ WPSOLR_Query_Parameters::SEARCH_PARAMETER_RESULTS_POSITION ] ?? 0 ); // Algolia's position starts at '1'
