...
/**
 * Class WPSOLR_IndexAmazon_PersonalizeClient
 *
 */
class WPSOLR_Index_Amazon_Personalize_Client extends WPSOLR_AbstractIndexClient {
...
TAG;
	...
	protected function search_engine_client_prepare_documents_for_update( array $documents ) {
		...
		foreach ( $documents as &$document ) {
			 ...
			 */
			$document['CREATION_TIMESTAMP'] = $document['displaydate_dt_i'];

			/**
			 * https://docs.aws.amazon.com/aws-sdk-php/v3/api/api-personalize-events-2018-03-22.html#putitems
			 * https://docs.aws.amazon.com/personalize/latest/dg/importing-items.html#importing-items-cli-sdk
			 */
			$formatted_document[] = [ 'itemId' => (string) $id, 'properties' => json_encode( $document ), ];
		}
	...
	public function send_posts_or_attachments_to_solr_index( $documents, $is_error = false ) {
		...
		try {
			 ...
			 * https://docs.aws.amazon.com/aws-sdk-php/v3/api/api-personalize-events-2018-03-22.html#putitems
			 */
			$search_engine_client_data = $this->_create_search_engine_client_events();

			$dataset_arn = $this->config['extra_parameters'][ WPSOLR_Hosting_Api_Abstract::FIELD_NAME_FIELDS_INDEX_DATASET_ITEMS_ARN ];
			$search_engine_client_data->putItems( [
			...
			throw new \Exception( sprintf( "(Amazon_Personalize) \"%s\"\n", $e->getMessage() ) );
		}
	...
	protected function search_engine_client_delete_all_documents( $post_types = null, $site_id = '' ) {
		...
		if ( ( is_null( $post_types ) || empty( $post_types ) ) && ( empty( $site_id ) ) ) {
			...
			/**
			 * https://www.amazon_personalize.com/doc/api-reference/api-methods/delete-by/#examples
			 * https://www.amazon_personalize.com/doc/api-reference/api-parameters/filters/#examples
			 **/
			$bool_queries = [];
	...
	protected function search_engine_client_delete_document( $document_id, $model = null ) {
		...
		$this->get_search_index()->deleteObject( $document_id );
	}

	/**
	 * https://www.amazon_personalize.com/doc/guides/managing-results/refine-results/geolocation/how-to/filter-results-around-a-location/#dataset
	 *
	 * @inheritDoc
	 */
	public function get_geolocation_field_value( $field_name, $lat, $long ) {
		...
		return [
			'field_name'  => static::FIELD_NAME_GEOLOC, // Amazon_Personalize uses a default field geolocation
	...
	public function set_field_is_exist( array &$document_for_update, string $field_name, string $is_exists ) {
		// Manage the 'not exists': https://www.amazon_personalize.com/doc/guides/managing-results/refine-results/filtering/how-to/filter-by-null-or-missing-attributes/
	...
	protected function _transform_event_tracking( \WP_Post $post, array $event, string $event_label ) {
		/**
		 * Transform: https://www.amazon_personalize.com/doc/api-reference/api-methods/send-events/
		 */
	...
	protected function _transform_event_tracking_click_search_result( \WP_Post $post, array $event, array $transformed_event ) {
		...
		$position                       = 1 + intval( $event[ WPSOLR_Query_Parameters::SEARCH_PARAMETER_RESULTS_POSITION ] ?? 0 ); // Amazon_Personalize's position starts at '1'
