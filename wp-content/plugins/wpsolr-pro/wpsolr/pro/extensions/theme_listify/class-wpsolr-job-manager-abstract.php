 ...
 */
abstract class WPSOLR_Job_Manager_Abstract extends WPSOLR_Extension {
	...
	public function __construct() {
		...
		if ( ! is_blog_admin() && $this->get_option_is_replace_search() ) {
			...
			// Fix WP Job Manager error
			add_filter( 'get_job_listings_query_args', array(
				$this,
				'apply_proximity_filter'
			), 9, 2 ); // Before Listify filter
	...
	/**
	 * Fix error in WP Job Manager. See ticket https://wordpress.org/support/topic/search-by-regions-and-keywords-not-working-since-1-33-0/#post-11617634
	 *
	...
	function wpsolr_filter_solarium_document_for_update( array $document_for_update, $solr_indexing_options, $post, $attachment_body, WPSOLR_AbstractIndexClient $search_engine_client ) {
		...
		if ( ! empty( $document_for_update[ self::GEOLOCATION_LAT . WpSolrSchema::_SOLR_DYNAMIC_TYPE_S ] )
			...
			$geolocation = $search_engine_client->get_geolocation_field_value(
			...
			$document_for_update[ $geolocation['field_name'] ] = $geolocation['field_value'];
		}
	...
	public function job_manager_get_listings_args( $args ) {
		...
		// Prevent recalculating (wrong) found_posts in WP_Query->set_found_posts()
	...
	function query( $retval, $query ) {
		...
		$wpsolr_query->query['s']         = $this->get_query( $this->search_listings_args );
	...
	protected function get_query( $args ) {
		return ! isset( $args['search_keywords'] ) ? '' : $args['search_keywords'];
	}

	/**
	 * @param WPSOLR_Query $wpsolr_query
	 */
	abstract protected function add_sort( WPSOLR_Query $wpsolr_query );
	...
	public function wpsolr_action_query( $parameters ) {
		...
		if ( isset( $wp_query->query['tax_query'] ) && ! empty( $wp_query->query['tax_query'] ) ) {
			...
			foreach ( $wp_query->query['tax_query'] as $tax_query ) {
				...
				if ( $this->_is_taxonomy_condition_and( $tax_query ) ) {


					$search_engine_client->search_engine_client_add_filter_in_all_terms(
	 ...
	 *
	 */
	abstract function add_custom_filters( WPSOLR_AbstractSearchClient $search_engine_client );
	 ...
	 * @return bool
	 */
	protected function _is_taxonomy_condition_and( $tax_query ) {
		return ( ! empty( $tax_query['operator'] ) && ( 'AND' === $tax_query['operator'] ) );
	}
