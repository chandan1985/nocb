...
class WPSOLR_Option_GeoLocation extends WPSOLR_Extension {
...
TAG;
		...
		foreach ( WPSOLR_Service_Container::getOption()->get_option_index_custom_fields( true ) as $custom_field_name ) {
			...
			if ( self::_SOLR_DYNAMIC_TYPE_LATITUDE_LONGITUDE === WpSolrSchema::get_custom_field_solr_type( $custom_field_name ) ) {
				...
				foreach ( $results->get_results() as $document ) {
					...
					if ( property_exists( $document, $distance_field_name ) && $document->$distance_field_name ) {
					...
					if ( property_exists( $document, $distance_field_name ) || isset( $document->$distance_field_name ) ) {
						...
						$result .= sprintf( self::TEMPLATE_RESULTS_GEO_DISTANCE,
							...
							sprintf( $template_text, $distance_field_name_translated,
								is_numeric( $distance ) ? number_format( $distance, 2, '.', ' ' ) : '' )
						);
					}
				}
			}
	...
	public function get_is_geolocation( WPSOLR_Query $wpsolr_query ) {
		...
		return ( ! empty( $wpsolr_latitude ) && is_numeric( $wpsolr_latitude ) && ! empty( $wpsolr_longitude ) && is_numeric( $wpsolr_longitude ) );
	}
	...
	/**
	 * Create geolocation field content from lat and long fields
	 *
	...
	function wpsolr_filter_solarium_document_for_update( array $document_for_update, $solr_indexing_options, $post, $attachment_body, WPSOLR_AbstractIndexClient $search_engine_client ) {
		...
		foreach ( $document_for_update as $custom_field_name => $custom_field_value ) {
			if ( self::_SOLR_DYNAMIC_TYPE_LATITUDE_LONGITUDE === WpSolrSchema::get_custom_field_solr_type( $custom_field_name ) ) {
				//$is_exists = 'n';

				// Add geolocation fields to the fields
				$lat_long = explode( ',', ( (array) $custom_field_value )[0] );

				if ( ! empty( $lat_long[0] ) && ! empty( $lat_long[1] ) ) {
					//$is_exists = 'y';
					...
					if ( ! isset( $document_for_update[ $geolocation['field_name'] ] ) ) {
						// Do not override value already set in themes lat,long code
						$document_for_update[ $geolocation['field_name'] ] = $geolocation['field_value'];
					}
				}

				//$search_engine_client->set_field_is_exist($document_for_update, $custom_field_name, $is_exists);
			}
