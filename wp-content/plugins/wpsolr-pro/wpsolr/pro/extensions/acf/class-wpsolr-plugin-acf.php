 ...
 */
class WPSOLR_Plugin_Acf extends WPSOLR_Extension {
	...
	/**
	 * ACF field types 'jQuery'
	 */
	const ACF_FIELD_TYPE_GOOGLE_MAP = 'google_map';
	const ACF_FIELD_TYPE_DATE_PICKER = 'date_picker';
	const ACF_FIELD_TYPE_DATE_TIME_PICKER = 'date_time_picker';
	const ACF_FIELD_TYPE_TIME_PICKER = 'time_picker';
						...
						switch ( $field['type'] ) {
							...
							default:
								...
								foreach ( is_array( $field['value'] ) ? $field['value'] : [ $field['value'] ] as $field_value ) {
									switch ( $field['type'] ) {
										case self::ACF_FIELD_TYPE_DATE_PICKER:
											$custom_fields[ $field['name'] ][] = strtotime( $field_value ) * 1000; // unix time in ms
											break;
