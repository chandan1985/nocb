...
WPSOLR_Extension::require_once_wpsolr_extension( WPSOLR_Extension::EXTENSION_THEME_LISTINGPRO, true );

$extension_options_name = WPSOLR_Option::OPTION_THEME_LISTINGPRO;

$extension_options = WPSOLR_Service_Container::getOption()->get_option_theme_listingpro();
$is_theme_active   = WPSOLR_Extension::is_plugin_active( WPSOLR_Extension::EXTENSION_THEME_LISTINGPRO );

$theme_name    = "ListingPro";
$theme_link    = "https://themeforest.net/item/listingpro-multipurpose-directory-theme/19386460";
$theme_version = "";
?>
...
<div wdm-vertical-tabs-contentid="extension_groups-options" class="wdm-vertical-tabs-content wpsolr-col-9">
    <form action="options.php" method="POST" id='extension_groups_settings_form'>
		<?php
		settings_fields( 'extension_theme_listingpro_opt' );
		?>
        ...
        <div class='wrapper'>
            ...
            <div class="wdm_row">
                ...
                <div class='col_right'>
                    <input type='checkbox' <?php WPSOLR_Escape::echo_escaped( $is_theme_active ? '' : 'readonly'); ?>
                           name='<?php WPSOLR_Escape::echo_esc_attr( $extension_options_name ); ?>[<?php WPSOLR_Escape::echo_esc_attr( WPSOLR_Option::OPTION_THEME_LISTINGPRO_IS_REPLACE_LISTING_SEARCH); ?>]'
                           value='y'
						<?php checked( isset( $extension_options[ WPSOLR_Option::OPTION_THEME_LISTINGPRO_IS_REPLACE_LISTING_SEARCH ] ) ); ?>>
                </div>
                <div class="clear"></div>
            </div>
            ...
            <div class='wdm_row'>
                <div class="submit">
					<?php if ( ! $license_manager->is_installed || $license_manager->get_license_is_activated( OptionLicenses::LICENSE_PACKAGE_LISTINGPRO ) ) { ?>
                        <div
                                class="wpsolr_premium_block_class"><?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, OptionLicenses::LICENSE_PACKAGE_LISTINGPRO, OptionLicenses::TEXT_LICENSE_ACTIVATED, true ) ); ?></div>
                        <input
                                ...
                                value="Save Options"/>
					<?php } else { ?>
						<?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, OptionLicenses::LICENSE_PACKAGE_LISTINGPRO, 'Save Options', true ) ); ?>
                        <br/>
					<?php } ?>
                </div>
            </div>
