...
namespace wpsolr\pro\extensions\theme_listingpro;
...
/**
 * Class WPSOLR_Theme_ListingPro
 *
 * Manage ListingPro theme
 */
class WPSOLR_Theme_ListingPro extends WPSOLR_Extension {
	...
	/**
	 * WPSOLR_Theme_ListingPro constructor.
	 */
	...
	public function wpsolr_action_query( $parameters ) {
		...
		$search_engine_client->search_engine_client_set_start( $wpsolr_query->wpsolr_get_start() );
		...
		// Convert ListingPro sort to wpsolr sort
		switch ( sprintf( '%s-%s', $order_by, $order ) ) {
			case 'date-ASC':
				$search_engine_client->search_engine_client_add_sort( WpSolrSchema::_FIELD_NAME_POST_DATE_GMT, WPSOLR_AbstractSearchClient::SORT_ASC );
				break;

			case 'date-DESC':
				$search_engine_client->search_engine_client_add_sort( WpSolrSchema::_FIELD_NAME_POST_DATE_GMT, WPSOLR_AbstractSearchClient::SORT_DESC );
				break;

			case 'title-ASC':
				$search_engine_client->search_engine_client_add_sort( WpSolrSchema::_FIELD_NAME_TITLE_S, WPSOLR_AbstractSearchClient::SORT_ASC );
				break;

			case 'title-DESC':
				$search_engine_client->search_engine_client_add_sort( WpSolrSchema::_FIELD_NAME_TITLE_S, WPSOLR_AbstractSearchClient::SORT_DESC );
				break;
		...
		foreach ( $taxonomies as $url_parameter_name => $taxonomy_name ) {
			...
			if ( ! empty( $url_parameters[ $url_parameter_name ] ) ) {
				...
				$search_engine_client->search_engine_client_add_filter_in_terms(
					sprintf( 'WPSOLR_Theme_ListingPro taxonomy %s', $taxonomy_name ),
					$field_name,
					$terms
				);
			}
