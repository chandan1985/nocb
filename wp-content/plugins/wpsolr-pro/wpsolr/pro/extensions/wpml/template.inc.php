    ...
    <form action="options.php" method="POST" id='extension_settings_form'>
        ...
        <div class='wrapper'>
            ...
            <div class="wdm_note">
                ...
                with <?php WPSOLR_Escape::echo_esc_html( $plugin_name ); ?> plugin.
                <br/>
            ...
            <div class="wdm_row">
                <div class='col_left'>
                    Use the
					<?php WPSOLR_Escape::echo_escaped( WPSOLR_Help::get_help( $plugin_help_link ) ); ?>
                    <a
                            href="<?php WPSOLR_Escape::echo_esc_url( $plugin_link ); ?>"
                            target="_blank"><?php WPSOLR_Escape::echo_esc_html( $plugin_name ); ?>
                        plugin <?php WPSOLR_Escape::echo_esc_html( $plugin_version ); ?></a>
			...
			<?php if ( $is_plugin_active ) { ?>
				...
				foreach ( $solr_indexes as $solr_index_indice => $solr_index ) {
					?>
                    <div class="wdm_row wpsolr_index_<?php WPSOLR_Escape::echo_esc_attr( $loop ); ?>">
                        <div class='col_left'>
                            Solr index '<?php WPSOLR_Escape::echo_esc_html( $solr_index['index_name'] ); ?>'
                        </div>
                        <div class='col_right'>

                            Is indexing: &nbsp;
                            <select class="wpsolr_index_language_code"
                                    name='<?php WPSOLR_Escape::echo_esc_attr( $extension_options_name ); ?>[solr_index_indice][<?php WPSOLR_Escape::echo_esc_attr( $solr_index_indice ); ?>][indexing_language_code]'>
								...
								foreach ( $languages as $language_code => $language ) {
									...
									WPSOLR_Escape::echo_escaped( sprintf( "<option value='%s' %s>%s</option>",
											WPSOLR_Escape::esc_attr( $language_code ),
											selected( $language_code, isset( $extension_options['solr_index_indice'][ $solr_index_indice ]['indexing_language_code'] )
												...
												: '' ),
											WPSOLR_Escape::esc_html( $language_code ) )
									);
                            ...
                            <input type='checkbox' class="wpsolr_index_is_default_search"
                                   name='<?php WPSOLR_Escape::echo_esc_attr( $extension_options_name ); ?>[solr_index_indice][<?php WPSOLR_Escape::echo_esc_attr( $solr_index_indice ); ?>][is_default_search]'
                                   value='1'
								<?php
				...
				WPSOLR_Escape::echo_escaped( $each_language_has_a_unique_solr_index
					? ''
                ...
                <div class="submit">
					<?php if ( ! $license_manager->is_installed || $license_manager->get_license_is_activated( $package_name ) ) { ?>
                        <div class="wpsolr_premium_block_class">
							<?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, $package_name, OptionLicenses::TEXT_LICENSE_ACTIVATED, true ) ); ?>
                        </div>
                        <input
                                ...
                                value="Save Options"/>
					<?php } else { ?>
						<?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, $package_name, 'Save Options', true ) ); ?>
                        <br/>
					<?php } ?>
                </div>
            </div>
