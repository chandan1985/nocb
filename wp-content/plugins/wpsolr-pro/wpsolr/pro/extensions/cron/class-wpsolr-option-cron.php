 ...
 * Manage crons
 */
class WPSOLR_Option_Cron extends WPSOLR_Extension {
	...
	public function wp_action_parse_request( &$wp ) {
		...
		if ( array_key_exists( self::WPSOLR_PATH_CRON, $wp->query_vars ) ) {
			...
			WPSOLR_Escape::echo_esc_json( $json_message );
	...
	private function save_cron_index_log( $cron_uuid, $index_uuid, $message ) {
		...
		if ( ! empty ( $cron_uuid ) && ! empty ( $index_uuid ) && ! empty ( $message ) ) {

			$option_logs_name = WPSOLR_Option_Cron::get_option_cron_indices_log_name( $cron_uuid );
			$logs             = get_option( $option_logs_name, [] );

			if ( ! is_array( $logs ) ) {
				$logs = [];
			}

			$logs[ $index_uuid ] = json_encode( $message, JSON_PRETTY_PRINT );

			update_option( $option_logs_name, $logs );
		}
	...
	private function save_cron_log( $cron_uuid, $message ) {
		...
		if ( ! empty( $cron_uuid ) && ! empty ( $message ) ) {

			update_option( WPSOLR_Option_Cron::get_option_cron_log_name( $cron_uuid ), json_encode( $message, JSON_PRETTY_PRINT ) );
		}
	 ...
	 * @return string
	 */
	public static function get_option_cron_log_name( string $cron_uuid ): string {
		return sprintf( '%s_%s', WPSOLR_Option::OPTION_CRON_LOG, $cron_uuid );
	}
	 ...
	 * @return string
	 */
	public static function get_option_cron_indices_log_name( string $cron_uuid ): string {
		return sprintf( '%s_%s', WPSOLR_Option::OPTION_CRON_LOG_INDICES, $cron_uuid );
	}
}
