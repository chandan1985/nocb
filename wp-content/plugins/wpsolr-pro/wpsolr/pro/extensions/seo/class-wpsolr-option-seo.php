 ...
 */
abstract class WPSOLR_Option_Seo extends WPSOLR_Extension {

	/** Prefix on keyword search permalinks, so they can be recognized from same facets permalink */
	const PURE_KEYWORD_PERMALINK_PREFIX = 's-';
	...
	/**
	 * Get Table schema: wpsolr_permalinks - Table for storing permalinks
	 *
	 * @return string
	 */
...
CREATE TABLE $this->table_name (
  ...
  KEY time (time)
	...
	/**
	 * Activate noindex/nofollow metas if current page url contains WPSOLR:
	 * - filters
	 * - sorts
	 * - ...
	 *
	...
	function wpsolr_filter_url_parameters(
		...
		if ( ! $this->is_permalink_url && ! empty( $url_parameters ) ) {
			$this->add_seo_filters_robots();
		}
	}
	...
	public function wpsolr_filter_redirect_search_home( $redirect_home ) {
		...
		if ( $this->get_is_permalinks_replace_search() ) {
			return sprintf( '%s/%s', $this->get_permalinks_home(), self::PURE_KEYWORD_PERMALINK_PREFIX );
		}
	...
	protected function get_permalinks_home() {
		 ...
		 */
		if ( is_multisite() ) {
			$blog_path = get_blog_details()->path;// "/site1/" if multisite subdirectory or "" if not multisite or multisite subdomain
			if ( ! empty( $permalink_home ) && ! empty( $blog_path ) ) {
				$permalink_home = trim( sprintf( '%s%s', $blog_path, $permalink_home ), '/' );
			}
		}
	...
	protected function is_generate_facet_permalinks() {
		...
		return $this->is_permalink_url &&
		       $this->get_container()->get_service_option()->get_option_seo_common_is_generate_facet_permalinks(
			       $this->get_option_name( $this->get_extension_name() )
		       );
	}
	...
	public function get_stored_permalink_query_parameters( $permalink_url ) {
		...
		if ( $this->get_container()->get_service_wpsolr()->starts_with( $permalink_url, self::PURE_KEYWORD_PERMALINK_PREFIX ) ) {
			// Pure keyword search permalink. Remove the prefix first.

			$permalink_query = [
				sprintf( '%s=%s', WPSOLR_Query_Parameters::SEARCH_PARAMETER_Q,
					WPSOLR_Regexp::remove_string_at_the_begining( $permalink_url, self::PURE_KEYWORD_PERMALINK_PREFIX ) )
			];
		...
		if ( $permalink_query ) {
			...
			$permalink_query_str = str_replace( '+', '%2B', $permalink_query_str );
	...
	public function replace_search() {
		...
		if ( false !== strpos( $url, '?s' ) ) {
			...
			$this->get_container()->get_service_wp()->wp_redirect(
				sprintf( '/%s/%s%s', $this->permalinks_home, empty( trim( $query ) ) ? '' : self::PURE_KEYWORD_PERMALINK_PREFIX, $query )
				, 302
			);
	...
	public function add_permalink_to_facets_data( $facets_data ) {
		...
		foreach ( $facets_data['facets'] as &$facet_data ) {
	...
	function add_permalink_to_facets_data_items(
		...
		foreach ( $facets_data_items as &$facet_item ) {
			...
			$facet_item['html_data']['is_permalink']   = true;
			$facet_item['html_data']['permalink_href'] = $facet_item['permalink']['href'] ?? '';
	...
	function generate_permalink(
		...
		if ( ! empty( $facet_name ) && ! empty( $facet_item ) && ! empty( trim( $facet_item['value'] ) ) ) {
		...
		if ( ! empty( $facet_name ) && ! empty( $facet_item ) ) {
			...
			if ( ! empty( $current_permalink[ WPSOLR_Query_Parameters::SEARCH_PARAMETER_FQ ] ) ) {
				// Permalinks of pure search keywords are not stored in SQL. Else duplicate issues between keyword and filter with same permalink.

				array_push( $values, $href_str, $this->generate_permalink_unique_query( $current_permalink ) );
			}

			/*$href_str = empty( $current_permalink[ WPSOLR_Query_Parameters::SEARCH_PARAMETER_FQ ] )
				? sprintf( '%s%s', self::PURE_KEYWORD_PERMALINK_PREFIX, $href_str )
				: $href_str;
			*/
	...
	function wpsolr_ajax_drop_permalinks_table() {
		...
		if ( ! isset( $_POST['security'] ) || ! wp_verify_nonce( $_POST['security'], WPSOLR_NONCE_FOR_DASHBOARD ) ) {
			...
			WPSOLR_Escape::echo_esc_json( wp_json_encode( $result ) );
	...
	/**
	 * Register the robots seo plugin filters
	 *
	 * @return mixed
	 */
	abstract function add_seo_filters_robots();
	...
	protected function generate_meta_robots( $robots ) {
		...
		$seo_robots = ( 0 === $this->nb_results )
			...
			: sprintf( self::META_ROBOTS, $this->is_content_noindex ? 'noindex' : 'index', $this->is_content_nofollow ? 'nofollow' : 'follow' );
