 ...
 */
class WPSOLR_Plugin_BbPress extends WPSOLR_Extension {
	...
	function __construct() {
		...
		add_action( 'bbp_update_forum_visibility', [ $this, 'bbp_update_forum_visibility' ], 10, 3 );

		add_action( WPSOLR_Events::WPSOLR_FILTER_POST_TYPES, [
			$this,
			'wpsolr_filter_post_types',
		], 999, 2 ); // applied after others, to be able to add forums non searchable post types

	}

	/**
	 * bbPress post types are not searchable. Make them searchable, unless the option is turned off.
	 * Applied after option premium filter (priority 99) that removes non searchable post types, to be able to add forums non searchable post types
	 *
	...
	/**
	 * Forums status are updated in DB directly by bbPress during the 'post_save', but the 'post_save' status is not modified.
	 * This made the forum status indexed wrong ('published' instead of 'private' or 'hidden').
	 *
	 * @param $forum_id
	 * @param $old_visibility
	 * @param $new_visibility
	 */
	function bbp_update_forum_visibility( $forum_id, $old_visibility, $new_visibility ) {
		$this->forum_status = $new_visibility;
	}
	...
	function add_fields_to_document_for_update( array $document_for_update, $solr_indexing_options, $post, $attachment_body, WPSOLR_AbstractIndexClient $search_engine_client ) {
		...
		if ( $this->forum_status ) {
			$document_for_update[ WpSolrSchema::_FIELD_NAME_STATUS_S ] = $this->forum_status;
			unset( $this->forum_status );
		}
	...
	public function wpsolr_filter_is_replace_by_wpsolr_query( $is_replace_by_wpsolr_query ) {
		$result = $is_replace_by_wpsolr_query || bbp_is_search() || bbp_is_search_results();
