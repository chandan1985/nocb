...
WPSOLR_Extension::require_once_wpsolr_extension( WPSOLR_Extension::EXTENSION_JET_ENGINE, true );

$extension_options_name = WPSOLR_Option::OPTION_EXTENSION_JET_ENGINE;

$extension_options = WPSOLR_Service_Container::getOption()->get_option_jet_engine();
$is_plugin_active  = WPSOLR_Extension::is_plugin_active( WPSOLR_Extension::EXTENSION_JET_ENGINE );
...
<div wdm-vertical-tabs-contentid="extension_groups-options" class="wdm-vertical-tabs-content wpsolr-col-9">
    <form action="options.php" method="POST" id='extension_settings_form'>
		<?php
		settings_fields( 'extension_jet_engine_opt' );
		?>
        ...
        <div class='wrapper'>
            ...
            <div class='wdm_row'>
                <div class="submit">
					<?php if ( ! $license_manager->is_installed || $license_manager->get_license_is_activated( OptionLicenses::EXTENSION_JET_ENGINE ) ) { ?>
                        <div class="wpsolr_premium_block_class">
							<?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, OptionLicenses::EXTENSION_JET_ENGINE, OptionLicenses::TEXT_LICENSE_ACTIVATED, true ) ); ?>
                        </div>
                        <input
                                ...
                                value="Save Options"/>
					<?php } else { ?>
						<?php WPSOLR_Escape::echo_escaped( $license_manager->show_premium_link( true, OptionLicenses::EXTENSION_JET_ENGINE, 'Save Options', true ) ); ?>
                        <br/>
					<?php } ?>
                </div>
            </div>
