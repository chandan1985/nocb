...
use wpsolr\pro\includes\readmetxt\WPSOLRPro_Readme_Txt;
...
if ( ! class_exists( WPSOLR_Pro_Updates::class ) ) {
	...
	class WPSOLR_Pro_Updates {
		...
		public function write_plugin_update_error_message( $plugin_data, $response ) {
			...
			$last_update_error = WPSOLR_Service_Container::getOption()->get_update_last_error();
			if ( ! empty( $last_update_error ) ) {
				$url      = admin_url( WPSOLR_Admin_Utilities::ADMIN_PAGE_EXTENSIONS );
				$redirect = sprintf( '<a href="%s" target="_blank">%s</a>', $url, 'extensions' );

				WPSOLR_Escape::echo_escaped( '<div class="wc_plugin_upgrade_notice">' );
				WPSOLR_Escape::echo_escaped( sprintf( '%s', WPSOLR_Escape::esc_html( $last_update_error ) ) );
				WPSOLR_Escape::echo_escaped( sprintf( self::REMOTE_ERROR_MESSAGE, WPSOLR_Escape::esc_html( $redirect ) ) );
