...
class SlideDeckSource_Facebook extends SlideDeck {
    ...
    function get_slides_nodes( $slidedeck ){
		...
		// $access_token_url = "http://www.slidedeck.com/fbapi.php?get_token=true";
	...
	// Get the appropriate Parent ID of the post, in case this is an autosave or revision - used for proper meta storage
        ...
        /*
        // check if cache access token
		$token_cache_key = $slidedeck_id . 'accesstoken' . $slidedeck['options']['cache_duration'] . $this->name;
        $access_token = slidedeck2_cache_read( $token_cache_key );
		
		if( !$access_token ){
			// fetch access token and set the cache
			$token_response = wp_remote_get( $access_token_url );
			if ( $token_response['response']['code'] === 200 ){
                            $access_token = json_decode($token_response['body'], true);    
                            $access_token = $access_token['access_token'];
                            // $access_token = str_replace("access_token=", "", $token_response['body']);
			} else {
				$access_token = false;
			}
			slidedeck2_cache_write( $token_cache_key, $access_token, $slidedeck['options']['cache_duration'] );
		}
	*/	
        //$feed_url = 'https://graph.facebook.com/' . $facebook_page_name . '/' . $feed_type . '?access_token=' . $access_token . '&limit=' . $slidedeck['options']['total_slides'];
		
		// update as per new api version 2.4
		
		$required_fields = array(
			'id','from','message','picture','link','type','created_time','updated_time','shares','likes','comments','story','caption','name','object_id'
		);
		
		// loop through all fields
		$feed_url .= '&fields=';
		foreach ( $required_fields as $field_key => $field_value ) {
			$feed_url .= $field_value.',';
		}
		// remove last comma
			...
			if( isset( $response_json->data ) ){
				foreach( (array) $response_json->data as $index => $entry ){
					...
					if( preg_match('/\/safe_image\.php.*&url=([^&]+)/', $results[ $index ]['image'], $matches ) ){
						...
						$bad_sources = array('fbstaging');
						$bad_sources_implode = implode('|', $bad_sources);
						if( preg_match("/$bad_sources_implode/", $matches[1], $matches_bad_sources ) ){
							$results[ $index ]['image'] = false;
						}

					}else{
						if( isset( $entry->object_id ) ){
							$results[ $index ]['image'] = 'https://graph.facebook.com/' . $entry->object_id . '/picture?type=normal';
						}
					}
					...
					$results[ $index ]['likes_count'] = isset( $entry->likes->count ) ? $entry->likes->count : false ;
					$results[ $index ]['author_name'] = $entry->from->name;
					$results[ $index ]['author_url'] = 'http://facebook.com/' . $entry->from->id;
					$results[ $index ]['author_avatar'] = 'https://graph.facebook.com/' . $entry->from->id . '/picture?type=large';
