...
class Bandwidth_Saver {
	...
	function checkCrawlerUA () {
		$this->bs_ipAddy = isset($_SERVER["REMOTE_ADDR"]) ? $_SERVER["REMOTE_ADDR"] : '';
		$this->bs_referrer = isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : '';
		$this->bs_userAgent = isset($_SERVER["HTTP_USER_AGENT"]) ? $_SERVER["HTTP_USER_AGENT"] : '';
		$this->bs_requestUri = isset($_SERVER["REQUEST_URI"]) ? $_SERVER["REQUEST_URI"] : '';
		$this->bs_queryString = isset($_SERVER["QUERY_STRING"]) ? $_SERVER["QUERY_STRING"] : '';
    ...
    function init() {
      	if (function_exists('load_plugin_textdomain')) {
			load_plugin_textdomain(
				'Bandwidth_Saver', 
				false,
				plugin_dir_path(__FILE__) . '/bandwidth-saver'
			);      	    
		}
    }
    ...
    function admin_menu() {
        $file = __FILE__;
        add_submenu_page('options-general.php', __('Chennai Central', 'Bandwidth_Saver'), __('Chennai Central','Bandwidth_Saver'), 'manage_options', $file, array($this, 'options_panel'));                
    }
	...
	function options_panel() {
		...
		*/
        ...
        $message_updated = __("Bandwidth saver Options Updated.", 'Bandwidth_Saver');
        // update options
        if (isset($_POST['action']) && $_POST['action'] == 'bs_update') {
...
// only do the 304 if NOT in the admin
if ( ! is_admin() ) {
