<?php
class SlideDeckSource_woocommerce extends SlideDeck {
    var $label = "WooCommerce";
    var $name = "woocommerce";
    var $default_lens = "tool-kit";
   var $taxonomies = array( 'product' );
    ...
    var $options_model = array(
	    ...
	    'show-price' => array(
                'type' => 'radio',
                'label' => "Show Price",
                'description' => "Turn on to show price for product",
                'data' => 'boolean',
                'value' => true,
                'attr' => array(
                	'class' => "fancy"
                ...
                'description' => "Turn on to show rating for product",
                'data' => 'boolean',
                'value' => true,
                'attr' => array(
                	'class' => "fancy"
	    ...
	    'price-position' => array(
                ...
                'type' => 'select',
                'values' => array(
                    'price-pos-lt' => 'Left-Top',
                    'price-pos-lb' => 'Left-Bottom',
                    'price-pos-rt' => 'Right-Top',
                    'price-pos-rb' => 'Right-Bottom'
                                  ),
		'attr' => array(
                    'class' => "fancy"
                ),
                'value' => 'price-pos-rb',
                'label' => 'Price Tag Position',
                'description' => "Choose where to place the Price position on the slide",
                'weight' => 50
            ),
	    'stock_status' => array(
                'name' => "stock_status",
                ...
                'label' => "Stock status",
                'description' => "Select products by its stock status.",
                'weight' => 25
            ),
	...
	add_action("{$this->namespace}_price_output", array( &$this, "slidedeck_woocommerce_price_output" ), 10, 1 );
    }
    

    function slidedeck_woocommerce_price_output($node){
	if($node['source'] == 'woocommerce'){
		echo '<span class="p_double p_inner">
			<div class = "sd-strikeout"></div>
			<div class = "accent-color">'.$node['regular_price'].'</div> 
			<div class = "accent-color">'.$node['sale_price'].'</div>
			</span>';
		}
    }
    /**
     ...
     * 
     * @uses SlideDeck::get()
     * @uses SlideDeckSource_woocommerce::_available_terms()
     */
    function ajax_available_terms() {
        ...
        $post_type = $_REQUEST['post_type'] = 'product';
     ...
     * 
     * @uses SlideDeck::get()
     * @uses SlideDeckSource_woocommerce::_available_filters()
     */
    ...
    function get_image( $slide, $slidedeck, $source = null, $tried_sources = array() ) {
		...
		if( $slidedeck['options']['postsImageSource'] == "default" )
		{
                          
                          $image_src =  SLIDEDECK2_URLPATH . '/sources/' . basename( dirname( __FILE__ ) ) . '/images/placeholder.png' ; 
                    ...
                    if( is_numeric( $slide['id'] ) ) {
                        ...
                        if( $thumbnail_id ) {
                            if( isset( $slidedeck['options']['image_scaling'] ) ) {
                                if( $slidedeck['options']['image_scaling'] == "default" ) {
                                    $image_size = "full";
                                }
                            }
       ...
       //set default image if product image is  not available.
       if( empty( $image_src ) ) 
       {
       	$image_src =  SLIDEDECK2_URLPATH . '/sources/' . basename( dirname( __FILE__ ) ) . '/images/placeholder.png' ; 
    ...
    function get_slides_nodes( $slidedeck ) {
        $post_type = $slidedeck['options']['post_type'] = 'product';
        ...
        if($slidedeck['options']['stock_status'] === "instock"){
        $query_args['meta_query'] = array(
            array(
                'key' => '_stock_status',
                'value' => $slidedeck['options']['stock_status']
            )
        );
	}
	...
	{
		$final_reg_price = $currency_symbol.$product_meta['_regular_price'][0];
		$final_sale_price = $currency_symbol.$product_meta['_sale_price'][0];
	}
	...
	{
		$final_reg_price = $product_meta['_regular_price'][0].$currency_symbol;
		$final_sale_price = $product_meta['_sale_price'][0].$currency_symbol;
	}
	else if($currency_pos === 'right_space')
	{
		...
		$final_sale_price = $product_meta['_sale_price'][0]." ".$currency_symbol;
            ...
            $slide = array(
                ...
                'regular_price' => $final_reg_price,
                'sale_price' =>$final_sale_price,
                'stock_status'=>$product_meta['_stock_status'][0],
                'show-price'=>   ($slidedeck['options']['show-price'] == '1') ? '1' : '0',
                'price-position'=> $slidedeck['options']['price-position'],
    ...
    function slidedeck_form_content_source( $slidedeck, $source ) {
        ...
        $image_sources['default'] = "Default Image";
	// Check if Woocommerce is activated
        global $woo_active; 
	global $at_least_one_product;
	$at_least_one_product = $woo_active = false; 
        
        if( is_plugin_active('woocommerce/woocommerce.php') ) {
         $woo_active = true;
          if($slidedeck['options']['total_slides'] > 0)
