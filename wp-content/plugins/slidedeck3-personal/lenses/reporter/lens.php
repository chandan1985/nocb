		...
		if( $this->is_valid( $slidedeck['lens'] ) ) {
	        ...
	        if( isset( $nodes['unfiltered_created_at'] ) && !empty( $nodes['unfiltered_created_at'] ) ) {
	            ...
	            switch( $date_format ) {
	                ...
	                case "european":
        	            $nodes['nav_created_at'] = date( "jS F, Y", $nodes['unfiltered_created_at'] );
                	break;
			...
			// trim the content
			if($slidedeck['options']['titleLengthWithImages'] <= strlen($nodes['title']))
                    {
			if( (int)$slidedeck['options']['width'] <= 500 ){
				$nodes['title'] = substr($nodes['title'], 0, 100).'...';	
			} else {
				$nodes['title'] = substr($nodes['title'], 0, 100).'...';	
			}
		     }
