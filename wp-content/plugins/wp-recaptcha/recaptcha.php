...
class ReCAPTCHAPlugin extends WPPlugin
{
    ...
    function register_actions() {
		...
		add_action('preprocess_comment', array(&$this, 'validate_recaptcha_field'), 0);
	...
	//added new validation for empty captcha - hemant
    function validate_recaptcha_field($comment_data) {
		if (empty($_POST['g-recaptcha-response']) ||
            $_POST['g-recaptcha-response'] == '') {
            wp_die('<body id="error-page"><p></p><p><strong>ERROR</strong>: CAPTCHA should not be empty</p><p></p><p><a href="javascript:history.back()">« Back</a></p></body>');
            return null;
        }
		return $comment_data;
	}
    ...
    function validate_recaptcha_response_wpmu($result) {
		...
		if( !empty($result) && is_admin()){ return true; } 
...
COMMENT_FORM;
        ...
        $escaped_error = isset($_GET['rerror']) ? htmlentities($_GET['rerror'], ENT_QUOTES) : '';
