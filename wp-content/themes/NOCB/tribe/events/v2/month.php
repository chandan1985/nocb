    ...
    if(!empty($all_Oldevents)){
      ?>
        ...
        <div id="tribe-events-footer">
          ...
          <a href="<?php print $current_url ?>/list/?eventDisplay=past" rel="prev" data-uw-rm-brl="false"><span>«</span> Previous Events</a>
        </div>
        <!-- End event footer -->
      <?php
    }
...
function event_pager(){
    pageSize = 12;
    var pageCount =  $(".events_class .event-item").length / pageSize;
    if(pageCount > 1){
        for(var i = 0 ; i<pageCount;i++){
            $("#events-pager").append('<li><a href="javascript:void(0)">'+(i+1)+'</a></li> ');
            if(i>pageSize){
            $("#events-pager li").eq(i).hide();
            }
        }
    }
    $("#events-pager li").first().find("a").addClass("current");
    slide = function(sens){
        $("#events-pager li").hide();
    }
    showPage = function(page) {
        $(".events_class .event-item").hide();
        $(".events_class .event-item").each(function(n) {
            if (n >= pageSize * (page - 1) && n < pageSize * page)
                $(this).show();
        });        
    }  
    showPage(1);
    $("#events-pager li a").eq(0).addClass("current");
    $("#events-pager li a").click(function() {
        $("#events-pager li a").removeClass("current");
        $(this).addClass("current");
        showPage(parseInt($(this).text()));
    });
}
jQuery(document).ajaxStop(function () {
    jQuery('ul#events-pager ').empty()
    event_pager();
});
event_pager();
</script>