    ...
    <?php if($featCounter == 1){ ?>
        ...
        <div class="block-50 block-h-295">
            ...
            foreach ($terms as $key => $value) {
               $homepage_display = get_field('homepage_display', 'digital_guide_' .$value->term_id);
               if ($homepage_display == "yes")
                $home_term = $value->term_id;
            }
            
            if ($home_term) { 
                 $activeimage = get_field('featured_image', 'digital_guide_' .$home_term); ?>
            <?php $cat_title = get_field('featured_title', 'digital_guide_' .$home_term); ?>
            <?php $term_link = get_term_link( $home_term ); ?>
            <?php $activeimagepos = "center center"; ?>
            <a class="hp-featured-img-link hp-featured-sub-img-link" style="overflow:hidden; background-position:<?php echo $activeimagepos; ?>; background-image:url(<?php echo $activeimage['url']; ?>);" href="<?php echo $term_link; ?>">
                <div class="hp-featured-img-overlay"></div>
                <h2 class="hp-featured-title"><?php echo $cat_title; ?></h2>
                <div class="hp-featured-expand"><img src="<?php print IMAGES;?>/expand-icon.png" /></div>
                <!-- <div class="hp-featured-summary"><?php //echo limit_words(get_the_excerpt(),20); ?>...</div> -->
                <div class="hp-featured-readmore">READ MORE</div>
            </a>
    ...
    <?php } else if($featCounter == 5){ ?>
        <?php if($sponsorQuery->have_posts()){ ?>
            <div class="block-25 block-h-41">
                <a class="hp-featured-img-link hp-featured-sub-img-link" style="overflow:hidden; background-position:<?php echo $storyActiveImagePos4; ?>; background-image:url(<?php echo $storyActiveImage4; ?>);" href="<?php echo $storyPermalink4; ?>">
                    <div class="hp-featured-img-overlay"></div>
                    <h2 class="hp-featured-title"><?php echo $storyTitle4; ?></h2>
                    <div class="hp-featured-expand"><img src="<?php print IMAGES;?>/expand-icon.png" /></div>
                    <div class="hp-featured-summary"><?php echo $storySummary4; ?>...</div>
                    <div class="hp-featured-readmore">READ MORE</div>
                </a>
            </div>
        <?php } else{ ?>
