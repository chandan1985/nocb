...
function tie_banner( $banner , $before= false , $after = false){
	// the following action is designed as a sort of remote overide to allow external plugins or whatever to take command of Jarida's internal banners
 	//  it works by sending the value of $banner to the filter, if that value is modified by a hook the modified content is echoed out to the screen
	$rval = apply_filters('tie_banner', $banner);
	if ($rval != $banner) {
		echo $rval;
		return;
	}
