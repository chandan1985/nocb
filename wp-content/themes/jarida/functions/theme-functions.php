...
function tie_setup() {
	...
	register_nav_menus( array(
		...
		'mobile' => __( 'Mobile Navigation', 'tie' ),
		'mobile-footer' => __( 'Mobile Footer Navigation', 'tie' )
	) );
...
function tie_get_wide_home_cats($cat_data){
	...
	switch( $cat_data['type'] ){
		...
		<div class="<?php echo $div_label;?>"><?php if( !empty( $cat_data['text'] ) ) echo do_shortcode( htmlspecialchars_decode(stripslashes ($cat_data['text']) )) ?></div>
		<div class="clear"></div>
		<?php
		break;
	}
...
function tie_get_social($newtab='yes', $icon_size='32', $tooltip='ttip'){
	...
	<div class="social-icons icon_<?php echo $icon_size; ?>">
		...
		// shutterfly
		if ( !empty($shutterfly) && $shutterfly != ' ' ) {
			?><a class="<?php echo $tooltip; ?>" title="shutterfly" href="<?php echo $shutterfly; ?>" <?php echo $newtab; ?>><i class="tieicon-shutterfly"></i></a><?php
		}
		// cart
		if ( !empty($cart) && $cart != ' ' ) {
			?><a class="<?php echo $tooltip; ?>" title="cart" href="<?php echo $cart; ?>" <?php echo $newtab; ?>><i class="tieicon-cart"></i></a><?php
		}
		// photo
		if ( !empty($photo) && $photo != ' ' ) {
			?><a class="<?php echo $tooltip; ?>" title="photo" href="<?php echo $photo; ?>" <?php echo $newtab; ?>><i class="tieicon-photo"></i></a><?php
		}
		// audio
		if ( !empty($audio) && $audio != ' ' ) {
			?><a class="<?php echo $tooltip; ?>" title="audio" href="<?php echo $audio; ?>" <?php echo $newtab; ?>><i class="tieicon-audio"></i></a><?php
		}
		// email
		if ( !empty($email) && $email != ' ' ) {
			?><a class="<?php echo $tooltip; ?>" title="email" href="<?php echo $email; ?>" <?php echo $newtab; ?>><i class="tieicon-email"></i></a><?php
		}
		// wikipedia
		if ( !empty($wikipedia) && $wikipedia != ' ' ) {
			?><a class="<?php echo $tooltip; ?>" title="wikipedia.com" href="<?php echo $wikipedia; ?>" <?php echo $newtab; ?>><i class="tieicon-wikipedia"></i></a><?php
		}
		?>
	</div>
...
function tie_show_extra_profile_fields( $user ) { ?>
	...
	<table class="form-table">
		...
		<tr>
			<th><label for="byline">Byline</label></th>
			<td>
				<input type="text" name="byline" id="byline" value="<?php echo esc_attr( get_the_author_meta( 'byline', $user->ID ) ); ?>" class="regular-text" /><br />
			</td>
		</tr>
	</table>
...
function tie_last_news_pic($order , $numberOfPosts = 12 , $cats = 1 ){
	...
	foreach($lastPosts as $post): setup_postdata($post); ?>
		...
		<div class="post-thumbnail">
			<a class="ttip" title="<?php the_title_attribute();?>" href="<?php the_permalink(); ?>" ><?php the_post_thumbnail( 'tie-small' ) ; ?></a>
		</div><!-- post-thumbnail /-->
	<?php endif; ?>
...
function tie_last_posts_cat($numberOfPosts = 5 , $thumb = true , $cats = 1){
	global $post;
	$orig_post = $post;
  //if(get_search_link($orig_post))
	if ( is_search() ) 
	{
		$lastPosts = get_posts('category='.$cats.'&suppress_filters=false&no_found_rows=1&numberposts='.$numberOfPosts);
	}
	else
	{
		$lastPosts = get_posts('category='.$cats.'&suppress_filters=0&no_found_rows=1&numberposts='.$numberOfPosts);
	}
	if(!empty($lastPosts) && is_array($lastPosts)) {
		foreach($lastPosts as $post){ 
			setup_postdata($post);
			?>
			<li <?php $ptype=get_post_type();if(!in_array($ptype, array('post','page'))){echo 'class="'.$ptype.'"';} ?>>
...
# Get Social Counter
	...
	function tie_facebook_fans( $page_link ){
		...
		if( $face_link['host'] == 'www.facebook.com' || $face_link['host']  == 'facebook.com' ){
			...
			if( empty( $fans ) ){
				try {
					$data = @json_decode(tie_remote_get( "https://graph.facebook.com/v2.5/$page_link?access_token=$fb_token&fields=likes"));
					$fans = $data->likes;
				} catch (Exception $e) {
					$fans = 0;
				}
