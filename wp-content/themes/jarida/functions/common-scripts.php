...
function tie_register() {
	...
	if(has_nav_menu( 'mobile' )){
		wp_register_script( 'mobilemenu', get_template_directory_uri() . '/js/mobilemenu.js', array( 'jquery' ), false, true );
		wp_register_style( 'mobilemenu', get_template_directory_uri().'/css/mobilemenu.css', array(), '', 'all' );
		wp_enqueue_style( 'mobilemenu' );
		wp_enqueue_script( 'mobilemenu' );
	}
	
	if(is_search()) {
		wp_enqueue_script("datepickerjs", "https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.3/jquery-ui.min.js");
		wp_enqueue_style( 'jquery-ui', 'https://code.jquery.com/ui/1.12.1/themes/smoothness/jquery-ui.css' );
		wp_enqueue_style( 'select2-css', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/css/select2.min.css' );
		wp_enqueue_script( 'select2-js', 'https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/js/select2.min.js', array( 'jquery' ), false, true  );
	}
	...
	wp_enqueue_style( 'override-css', get_template_directory_uri().'/css/override.css?unique='.time(), array(), time(), 'all' );
...
function tie_enqueue_font ( $got_font) {
	...
	if( $font_type == 'non-google' ){
		...
		$protocol = is_ssl() ? 'https' : 'http';
			...
			wp_enqueue_style( $font_name , '//fonts.googleapis.com/earlyaccess/'.$font_name); // Removing Protocol
			...
			wp_enqueue_style( $font_name , '//fonts.googleapis.com/css?family='.$font_name . ':' . $font_variants.$char_set );  // Removing Protocol
		}
	}
}
...
foreach( $custom_typography as $selector => $value){
	$option = tie_get_option( $value );
	if( (isset($option['font']) && $option['font'])
		|| (isset($option['color']) && $option['color']) 
		|| (isset($option['size']) && $option['size']) 
