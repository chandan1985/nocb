...
function posts_init(){
	...
	$config = unserialize(get_option('tdc_sponsored_content'));
		if(!empty($config['sponsored_option_byline']) && $config['sponsored_option_byline'] == 'yes') { 
			$post_types = array ( 'post', 'sponsored_content', 'page');
    		foreach( $post_types as $post_type )
    		{
		  		add_meta_box("post_options", theme_name ." - Post Options", "post_options", $post_types , "normal", "high");
	    	}
		}
...
?>
    ...
    <div class="tiepanel-item">
        ...
        <?php
        tie_post_options(
            array(	"name" => "Hide Title",
                "id" => "tie_hide_title",
                "type" => "checkbox"));

        tie_post_options(
            array(	"name" => "Hide Comments",
                "id" => "tie_hide_comments",
                "type" => "checkbox"));
        tie_post_options(
            array(	"name" => "Hide Breadcrumbs",
                "id" => "tie_hide_breadcrumbs",
                "type" => "checkbox"));
        ?>
    </div>
		...
		<div class="tiepanel-item" id="tie-template-blog">
			...
			<div class="option-item">
				...
				<select multiple="multiple" name="tie_blog_cats[]" id="tie_blog_cats">
					<?php
					 $tie_blog_cats = isset($get_meta["tie_blog_cats"][0]) ? unserialize($get_meta["tie_blog_cats"][0]) : array();
		...
		<div class="tiepanel-item" id="tie-template-authors">
			...
			<div class="option-item">
					...
					<select multiple="multiple" name="tie_authors[]" id="tie_authors">
						<?php
						 $tie_authors = isset($get_meta["tie_authors"][0]) ? unserialize($get_meta["tie_authors"][0]) : array();
...
?>
	...
	switch ( $value['type'] ) {
		...
		?>
				<input id="<?php echo $value['id']; ?>-img" class="img-path" type="text" size="56" style="direction:ltr; text-align:left" name="<?php echo $value['id']; ?>[img]" value="<?php echo isset($current_value['img']) ? $current_value['img'] : ''; ?>" />
				...
				<div style="margin-top:15px; clear:both">
					...
					<input style="width:80px; margin-right:5px;"  name="<?php echo $value['id']; ?>[color]" id="<?php  echo $value['id']; ?>color" type="text" value="<?php echo isset($current_value['color']) ? $current_value['color'] : ''; ?>" />
					<?php $repeat = isset($current_value['repeat']) ? $current_value['repeat'] : ''; ?>
					...
					<?php $attachment = isset($current_value['attachment']) ? $current_value['attachment'] : ''; ?>
					<select name="<?php echo $value['id']; ?>[attachment]" id="<?php echo $value['id']; ?>[attachment]" style="width:96px;">
						<option value="" ></option>
						<option value="fixed" <?php selected($attachment,'fixed'); ?>>Fixed</option>
						<option value="scroll" <?php selected($attachment,'scroll'); ?>>scroll</option>
					</select>
					<?php $hor = isset($current_value['hor']) ? $current_value['hor'] : ''; ?>
					<select name="<?php echo $value['id']; ?>[hor]" id="<?php echo $value['id']; ?>[hor]" style="width:96px;">
						<option value=""></option>
						<option value="left" <?php selected($hor,'left'); ?>>Left</option>
						<option value="right" <?php selected($hor,'right'); ?>>Right</option>
						<option value="center" <?php selected($hor,'center'); ?>>Center</option>
					</select>
					
					<?php $ver = isset($current_value['ver']) ? $current_value['ver'] : ''; ?>
					<select name="<?php echo $value['id']; ?>[ver]" id="<?php echo $value['id']; ?>[ver]" style="width:100px;">
						<option value=""></option>
						<option value="top" <?php selected($ver,'top'); ?>>Top</option>
						<option value="center" <?php selected($ver,'center'); ?>>Center</option>
						<option value="bottom" <?php selected($ver,'bottom'); ?>>Bottom</option>
