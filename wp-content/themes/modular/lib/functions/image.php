...
function mysite_display_image( $args = array() ) {
	...
	if(!empty($img_id_check))
	{
		$image_info = wp_get_attachment_image_src( $img_id_check, '' );
		$image = $image_info[0];
				
		# Auto crop width
		if( empty( $no_preload_img ) && ( empty( $width ) || !is_numeric( $width ) ) ) {
			$width = ( !empty( $width ) ) ? $width : '';
			$width = mysite_auto_width( array( 'width' => $width, 'get_width' => $image_info[1] ) );
		}
		
		$width = ( !empty( $width ) ) ? trim(str_replace(' ', '', str_replace('px', '', $width ) ) ) : $image_info[1];
		$height = ( !empty( $height ) ) ? trim(str_replace(' ', '', str_replace('px', '', $height ) ) ) : $image_info[2];
	}
	else
	{
		# Auto crop width
