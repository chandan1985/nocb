...
<div class="container">
  <div class="section-title">
    <h2 class="river-heading section-title__heading">CityBusiness Events</h2>
  </div>
</div> <!-- .container ends -->
...
<div class="container">
  <div class="pbm-row">
  <div class="content-top"><?php dynamic_sidebar('content-top');  ?></div>
    <div class="pbm-col pbm-col-two-third-md post-river">
      ...
      <?php } ?>
          ...
          <!-- -->
          <div class="event_loader"> <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/event-loader-new.gif" alt="Loader" title="Loader" width="50"></div>
          <!-- -->
          <!-- needs to add container here -->
			...
			$excluded_category = get_option('myplugin_settings_excluded_category_callback_field');
            $excluded_category = explode(',', $excluded_category);
            foreach ($excluded_category as $categories) {
              $excluded_category_term = get_term_by('slug', $categories, 'tribe_events_cat'); 
              $excluded_category_slug_name = $excluded_category_term->slug;
              $excluded_category_id = $excluded_category_term->term_id;
              $excluded_category_ids[] = $excluded_category_term->term_id;
              $excluded_category_name = $excluded_category_term->name;
              $excluded_category_term_ids[] = $excluded_category_term->term_id; 
            }

			
            // $tribe_paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
            // $start = ($tribe_paged - 1) * 6;
            ...
            if(wp_is_mobile()){
              $args = array(
                ...
                array(
				 ...
				 'tax_query' => 
                  array (
                    'taxonomy_tribe_events_cat' => 
                    array (
                      'taxonomy' => 'tribe_events_cat',
                      'field' => 'id',
                      'terms' => $excluded_category_term_ids,
                      'operator' => 'NOT IN',
...
<?php if (isset($footer_hide_from_post_type) && is_array($footer_hide_from_post_type) &&
  ...
  <div class="row">
      <div class="col-sm-12">
          <div class='footer-bottom-ad-section text-center mb-4'>
              <?php
          if(wp_is_mobile()){
...
?>
 ...
 <div class="container">
  <div class="section-title">
    ...
    <div class="section-content"><?php require get_template_directory() . '/inc/local-events.php'; ?></div>
  </div>
</div>
<?php
}
